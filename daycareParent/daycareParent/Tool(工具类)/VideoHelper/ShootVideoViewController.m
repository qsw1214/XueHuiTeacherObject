...
-(void)initCapture{
    ...
    finishBt = [[UIButton alloc]initWithFrame:CGRectMake(0, 0, 30, 30)];
    finishBt.center = CGPointMake(SCREEN_WIDTH-35, 64+preLayerHeight+20+45);
    finishBt.adjustsImageWhenHighlighted = NO;
    [finishBt setBackgroundImage:[UIImage imageNamed:@"37x-Checkmark.png"] forState:UIControlStateNormal];
    ...
    closeBtn = [[UIButton alloc]initWithFrame:CGRectMake(0, 0, 30, 30)];
    closeBtn.center = CGPointMake(40, 64+preLayerHeight+20+45);
    closeBtn.adjustsImageWhenHighlighted = NO;
    [closeBtn setBackgroundImage:[UIImage imageNamed:@"37x-Wrong.png"] forState:UIControlStateNormal];
        ...
        switch (exportStatus)
        {
            ...
            case AVAssetExportSessionStatusCompleted:
            {
                dispatch_async(dispatch_get_main_queue(), ^{
                    ...
                    if ([self.delegate respondsToSelector:@selector(videoStatrToMp4Finished:image:)])
                    {
                        [self.delegate videoStatrToMp4Finished:[NSData dataWithContentsOfFile:path] image:[self getVideoPreViewImage:mergeFileURL]];
                    }
                     [self dismissViewControllerAnimated:YES completion:nil];
                });
