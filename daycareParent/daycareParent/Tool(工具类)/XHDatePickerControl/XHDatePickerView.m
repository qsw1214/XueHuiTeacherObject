...
typedef NS_ENUM(NSInteger,DatePickerComponentsType)
{
    DatePickerComponentsYear = 1,
    DatePickerComponentsMonth = 2,
    DatePickerComponentsDay = 3,
    DatePickerComponentsHour = 4,
    DatePickerComponentsMinute = 5,
    DatePickerComponentsSecond = 6,
};


@interface XHDatePickerView () <UIPickerViewDelegate, UIPickerViewDataSource>
...
- (void)show {
    ...
    self.month = [NSString stringWithFormat:@"%zd月", [self.timeArr[1] integerValue]];
    self.day = [NSString stringWithFormat:@"%zd日", [self.timeArr[2] integerValue]];
    self.hour = [NSString stringWithFormat:@"%zd时", [self.timeArr[3] integerValue]];
...
{
    NSLog(@"点击了保存");
    NSString *month = self.month.length == 3 ? [NSString stringWithFormat:@"%zd", self.month.integerValue] : [NSString stringWithFormat:@"0%zd", self.month.integerValue];
    NSString *day = self.day.length == 3 ? [NSString stringWithFormat:@"%zd", self.day.integerValue] : [NSString stringWithFormat:@"0%zd", self.day.integerValue];
        ...
        case 1: { // 月
            ...
            if ([self.year integerValue] > [self.timeArr[0] integerValue]) {
                ...
                if (month_value.integerValue < [self.timeArr[component] integerValue]) {
                    [pickerView selectRow:[self.dataArray[component] indexOfObject:[NSString stringWithFormat:@"%zd月", [self.timeArr[component] integerValue]]] inComponent:component animated:YES];
            ...
            if ([self.year integerValue] > [self.timeArr[0] integerValue]) {
                ...
                if ([self.month integerValue] > [self.timeArr[1] integerValue]) {
                    ...
                    if (day_value.integerValue < [self.timeArr[component] integerValue]) {
                        ...
                        [pickerView selectRow:[self.dataArray[component] indexOfObject:[NSString stringWithFormat:@"%zd日", time_integerValue]] inComponent:component animated:YES];
                        // 如果选择的日大于当前日，就复制日
                    } else {
                        if (self.dayArr.count <= [dayStr integerValue]) {
...
-(NSInteger)getComponentsType:(DatePickerComponentsType)type
{
    ...
    switch (type)
    {
        case DatePickerComponentsYear:
        {
            return comp.year;
        }
            break;
        case DatePickerComponentsMonth:
        {
            return comp.month;
        }
            break;
        case DatePickerComponentsDay:
        {
            return comp.day;
        }
            break;
        case DatePickerComponentsHour:
        {
            return comp.hour;
        }
            break;
        case DatePickerComponentsMinute:
        {
            return comp.minute;
        }
            break;
        case DatePickerComponentsSecond:
        {
            return comp.second;
        }
            break;
