...
-(void)playRecord{
    void (^didFinishPlay)(void) = ^(){
        ...
        self.isPlaying = NO;
        [[XHVoicePlayer sharePlayer] endPlay];
    };
    
    if (!self.isPlaying) {
        [self.voicePlayingImageView startAnimating];
        self.isPlaying = YES;
        [[XHVoicePlayer sharePlayer] playVoiceWithData:((RCVoiceMessage *)self.message.content).wavAudioData didFinish:didFinishPlay];
    }else{
        didFinishPlay();
    }
}
